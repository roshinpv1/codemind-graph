...
- **Application graphs:** `application_graphs(loadable)` uses `graph_role=source` (legacy `ci` rows are treated as source), with a fallback when app was only under `test`.
...
- Project coverage route should use the same `application_graphs()` helper as synthesis so **`source`** (application) and **`test`** slots stay consistent.
