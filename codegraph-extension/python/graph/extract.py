...
def _pascal_resolve_unit(from_path: Path, unit_name: str) -> str:
    """Resolve a Pascal unit name to the graph node ID of its source file.
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from graph.detect import _load_graphignore, _is_ignored, _is_noise_dir
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m graph.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
