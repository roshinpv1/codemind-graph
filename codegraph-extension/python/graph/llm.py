...
# Gemini, and OpenAI.
# Used by `graph extract . --backend gemini` and the benchmark scripts.
# The default graph pipeline uses Claude Code subagents via skill.md;
...
BACKENDS: dict[str, dict] = {
    ...
    "gemini": {
        ...
        "model_env_key": "GRAPH_GEMINI_MODEL",
    ...
    "openai": {
        ...
        "model_env_key": "GRAPH_OPENAI_MODEL",
    ...
    "deepseek": {
        ...
        "model_env_key": "GRAPH_DEEPSEEK_MODEL",
    ...
    "bedrock": {
        ...
        "model_env_key": "GRAPH_BEDROCK_MODEL",
        "pricing": {"input": 3.0, "output": 15.0},  # USD per 1M tokens
        "temperature": 0,
        "max_tokens": 16384,
    },
...
def _resolve_max_tokens(default: int) -> int:
    """Honour GRAPH_MAX_OUTPUT_TOKENS env var override, else use backend default."""
    raw = os.environ.get("GRAPH_MAX_OUTPUT_TOKENS", "").strip()
    if raw:
        try:
            v = int(raw)
            if v > 0:
...
_EXTRACTION_SYSTEM = """\
You are a graph semantic extraction agent. Extract a knowledge graph fragment from the files provided.
...
def _parse_llm_json(raw: str) -> dict:
    ...
    if len(raw) > _LLM_JSON_MAX_BYTES:
        print(
            f"[graph] LLM response exceeds {_LLM_JSON_MAX_BYTES} bytes "
    ...
    try:
        ...
        print(f"[graph] LLM returned invalid JSON, skipping chunk: {exc}", file=sys.stderr)
        return {"nodes": [], "edges": [], "hyperedges": []}
...
def _call_openai_compat(
    ...
    *,
    ...
    try:
        ...
        pkg_hint = "graphy[kimi]" if backend == "kimi" else "openai"
        raise ImportError(
    ...
    # default. Honour GRAPH_API_TIMEOUT (seconds) for explicit override;
    ...
    timeout_raw = os.environ.get("GRAPH_API_TIMEOUT", "").strip()
    ...
    if backend == "ollama":
        num_ctx_raw = os.environ.get("GRAPH_OLLAMA_NUM_CTX", "").strip()
        ...
        if num_ctx_raw:
            try:
                ...
                print(
                    f"[graph] GRAPH_OLLAMA_NUM_CTX={num_ctx_raw!r} is not a valid integer; "
                ...
                if num_ctx < estimated_input:
                    print(
                        f"[graph] warning: GRAPH_OLLAMA_NUM_CTX={num_ctx} is smaller than "
            ...
            num_ctx = auto_num_ctx
        keep_alive = os.environ.get("GRAPH_OLLAMA_KEEP_ALIVE", "30m")
    ...
    if _response_is_hollow(raw_content, result) and result["finish_reason"] != "length":
        print(
            f"[graph] {backend or 'backend'} returned a hollow response "
...
def _call_claude(api_key: str, model: str, user_message: str, max_tokens: int = 8192) -> dict:
    ...
    if _response_is_hollow(raw_content, result) and result["finish_reason"] != "length":
        print(
            "[graph] claude returned a hollow response; treating as "
...
def _call_claude_cli(user_message: str, max_tokens: int = 8192) -> dict:
    ...
    a pay-as-you-go API key just to run graph's semantic pass.
    """
    import shutil
    import subprocess
    ...
    if _response_is_hollow(raw_content, result) and result["finish_reason"] != "length":
        print(
            "[graph] claude-cli returned a hollow response; treating as "
            "truncation so adaptive retry can bisect the chunk.",
            file=sys.stderr,
        )
        result["finish_reason"] = "length"
...
def _call_bedrock(model: str, user_message: str, max_tokens: int = 8192) -> dict:
    ...
    try:
        ...
        raise ImportError(
            "AWS Bedrock extraction requires boto3. Run: pip install graphy[bedrock]"
        ) from exc
    ...
    if _response_is_hollow(text, result) and result["finish_reason"] != "length":
        print(
            "[graph] bedrock returned a hollow response; treating as "
            "truncation so adaptive retry can bisect the chunk.",
            file=sys.stderr,
        )
        result["finish_reason"] = "length"
...
def extract_files_direct(
    ...
    if not key and backend == "ollama":
        ...
        print(
            "[graph] WARNING: ollama backend selected with no OLLAMA_API_KEY set; "
...
def _extract_with_adaptive_retry(
    ...
    try:
        ...
        if len(chunk) <= 1:
            print(
                f"[graph] single-file chunk {chunk[0]} exceeds model context "
        ...
        if _depth >= max_depth:
            print(
                f"[graph] chunk of {len(chunk)} still overflows context at "
                f"recursion depth {_depth} (max {max_depth}) — dropping",
                file=sys.stderr,
            )
            return {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0, "model": model, "finish_reason": "stop"}
        print(
            f"[graph] chunk of {len(chunk)} exceeded context at depth "
    ...
    if len(chunk) <= 1:
        print(
            f"[graph] single-file chunk {chunk[0]} truncated at "
    ...
    if _depth >= max_depth:
        print(
            f"[graph] chunk of {len(chunk)} still truncated at recursion "
    ...
    print(
        f"[graph] chunk of {len(chunk)} truncated at depth {_depth}, "
...
def extract_corpus_parallel(
    ...
    if backend == "ollama" and os.environ.get("GRAPH_OLLAMA_PARALLEL", "").strip() != "1":
        max_concurrency = 1
    ...
    if backend == "claude-cli" and os.environ.get("GRAPH_CLAUDE_CLI_PARALLEL", "").strip() != "1":
        max_concurrency = 1
    ...
    if workers == 1:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if exc is not None:
                print(f"[graph] chunk {idx + 1}/{total} failed: {exc}", file=sys.stderr)
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for future in as_completed(futures):
                ...
                if exc is not None:
                    print(
                        f"[graph] chunk {idx + 1}/{total} failed: {exc}",
                        file=sys.stderr,
                    )
                    merged["failed_chunks"] += 1
                    continue
    ...
    if merged["failed_chunks"] > 0:
        print(
            f"[graph] WARNING: {merged['failed_chunks']}/{total} semantic chunk(s) failed"
...
def _call_llm(prompt: str, *, backend: str, max_tokens: int = 200) -> str:
    ...
    Used by lightweight callers (e.g. `graph.dedup` LLM tiebreaker) that
    ...
    Previously `graph.dedup` imported a `_call_llm` symbol that did not
...
def _validate_ollama_base_url(url: str) -> None:
    ...
    try:
        ...
        print(
            f"[graph] WARNING: OLLAMA_BASE_URL={url!r} is not a parseable URL.",
            file=sys.stderr,
        )
        return
    if parsed.scheme not in ("http", "https"):
        print(
            f"[graph] WARNING: OLLAMA_BASE_URL has unexpected scheme {parsed.scheme!r}; "
    ...
    if not is_loopback:
        ...
        print(
            f"[graph] WARNING: OLLAMA_BASE_URL points to non-loopback host {host!r}{scheme_note}. "
