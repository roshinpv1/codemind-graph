...
def _community_article(
    ...
    lines += ["---", "", "*Part of the graph knowledge wiki. See [[index]] to navigate.*"]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by graph. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [graph](https://github.com/CodeGraph/codegraph)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `graph extract .` or `graph cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `graph extract .` to regenerate .graph_analysis.json."
        )
