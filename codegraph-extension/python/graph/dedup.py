"""Entity deduplication pipeline for graph knowledge graphs.
...
def deduplicate_entities(
    ...
    *,
    ...
    total = len(remap)
    msg = f"[graph] Deduplicated {total} node(s)"
    if exact_merges:
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from graph.llm import BACKENDS, _format_backend_env_keys, _get_backend_api_key
        if backend not in BACKENDS:
            print(f"[graph] --dedup-llm: unknown backend {backend!r}, skipping LLM tiebreaker.", flush=True)
            return
        if not _get_backend_api_key(backend):
            env_keys = _format_backend_env_keys(backend)
            print(f"[graph] --dedup-llm: {env_keys} not set, skipping LLM tiebreaker.", flush=True)
            return
    except ImportError:
        return
    ...
    try:
        from graph.llm import _call_llm
    except ImportError as exc:
        # F-038: previously this silent fallback hid the fact that `_call_llm`
        # didn't exist in `graph.llm` at all, so `--dedup-llm` was a no-op.
        # Surface the import failure so future regressions are visible.
        print(
            f"[graph] --dedup-llm: cannot import _call_llm ({exc}); skipping LLM tiebreaker.",
            flush=True,
        )
        return
    ...
    for batch_start in range(0, len(ambiguous), batch_size):
        ...
        try:
            ...
            print(f"[graph] --dedup-llm batch failed: {exc}", flush=True)
