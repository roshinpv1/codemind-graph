...
# shared-output setups. Accepts a relative name ("graph-out-feature") or an
# absolute path ("/shared/graph-out").
...
# Use `graph extract --force` to bypass when needed.
...
def cache_dir(root: Path = Path("."), kind: str = "ast") -> Path:
    """Returns graph-out/cache/{kind}/ - creates it if needed.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast") -> dict | None:
    ...
    Cache value: stored as graph-out/cache/{kind}/{hash}.json
    ...
    if kind == "ast":
        legacy = Path(root).resolve() / _GRAPH_OUT / "cache" / f"{h}.json"
        if legacy.exists():
            try:
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast") -> None:
    ...
    Stores as graph-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
