"""graph prs — graph-aware PR dashboard.
...
Usage:
  graph prs                   # dashboard of all open PRs
  graph prs <number>          # deep dive on one PR
  graph prs --triage          # Opus ranks your review queue
  graph prs --worktrees       # show worktree → branch → PR mapping
  graph prs --conflicts       # PRs sharing graph communities (merge-order risk)
  graph prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  graph prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def _resolve_triage_backend() -> tuple[str, str]:
    """Return (backend, model) using GRAPH_TRIAGE_BACKEND or first available key."""
    from graph.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend

    explicit = os.environ.get("GRAPH_TRIAGE_BACKEND", "").strip()
    if explicit in BACKENDS:
        model = (os.environ.get("GRAPH_TRIAGE_MODEL")
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    try:
        from graph.llm import BACKENDS, _get_backend_api_key
    except ImportError:
        print(red("  graph.llm not available — cannot run triage."), file=sys.stderr)
        sys.exit(1)
...
def cmd_prs(argv: list[str]) -> None:
    ...
    graph_path = Path("graph-out/graph.json")
