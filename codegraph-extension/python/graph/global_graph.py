...
def global_add(source_path: Path, repo_tag: str) -> dict:
    ...
    """
    from graph.build import prefix_graph_for_global, prune_repo_from_graph
    ...
    if existing_path and existing_path != str(source_path.resolve()):
        print(
            f"[graph global] warning: repo tag '{repo_tag}' previously pointed to "
...
def global_remove(repo_tag: str) -> int:
    ...
    from graph.build import prune_repo_from_graph
