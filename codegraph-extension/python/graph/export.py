...
from graph.analyze import _node_community_map
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .graph_semantic_marker is present (graph cost real LLM tokens), or
    - .graph_labels.json contains at least one non-default community label
    ...
    is_semantic = (out / ".graph_semantic_marker").exists()
    is_curated = False
        ...
        for name in _BACKUP_ARTIFACTS:
            ...
            if src.exists():
                try:
                    ...
                    pass
        if copied:
            print(f"[graph] backed up {reason} graph ({copied} files) → {backup_dir.name}/")
        return backup_dir
    except Exception as exc:
        import sys
        print(f"[graph] warning: backup failed ({exc}) — continuing with overwrite", file=sys.stderr)
        return None
...
def _yaml_str(s: str) -> str:
    ...
    See `graph.ingest._yaml_str` for the full rationale; duplicated here to
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /graph", ""]
...
def to_obsidian(
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"graph/{ftype}" if ftype else "graph/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"graph/{dom_conf}"
