"""graph CLI - `graph install` sets up the Claude Code skill."""
...
try:
    ...
    __version__ = _pkg_version("graphy")
except Exception:
    __version__ = "unknown"

# Output directory — override with GRAPH_OUT env var for worktrees or shared-output setups.
# Accepts a relative name ("graph-out-feature") or an absolute path ("/shared/graph-out").
_GRAPH_OUT = os.environ.get("GRAPH_OUT", "graph-out")


def _default_graph_path() -> str:
    return str(Path(_GRAPH_OUT) / "graph.json")


def _check_skill_version(skill_dst: Path) -> None:
    """Warn if the installed skill is from an older graph version."""
    try:
        version_file = skill_dst.parent / ".graph_version"
        if not version_file.exists():
            return
        if not skill_dst.exists():
            print("  warning: skill dir exists but SKILL.md is missing. Run 'graph install' to repair.")
            return
        installed = version_file.read_text(encoding="utf-8").strip()
        if installed != __version__:
            print(f"  warning: skill is from graph {installed}, package is {__version__}. Run 'graph install' to update.", file=sys.stderr)
    except Exception:
        pass


def _refresh_all_version_stamps() -> None:
    """After a successful install, update .graph_version in all other known skill dirs.
    ...
    for cfg in _PLATFORM_CONFIG.values():
        try:
            ...
            vf = skill_dst.parent / ".graph_version"
            if skill_dst.exists():
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                "  [ -f graph-out/graph.json ] && "
                r"""  echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"graph: knowledge graph at graph-out/. For focused questions, run `graph query \"<question>\"` (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context."}}' """
                "  || true ;; "
                "esac"
            ),
        }
...
_SKILL_REGISTRATION = (
    "\n# graph\n"
    "- **graph** (`~/.claude/skills/graph/SKILL.md`) "
    "- any input to knowledge graph. Trigger: `/graph`\n"
    "When the user types `/graph`, invoke the Skill tool "
    "with `skill: \"graph\"` before doing anything else.\n"
)
...
_PLATFORM_CONFIG: dict[str, dict] = {
    "claude": {
        "skill_file": "skill.md",
        "skill_dst": Path(".claude") / "skills" / "graph" / "SKILL.md",
        "claude_md": True,
    },
    "codex": {
        "skill_file": "skill-codex.md",
        "skill_dst": Path(".agents") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "opencode": {
        "skill_file": "skill-opencode.md",
        "skill_dst": Path(".config") / "opencode" / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "aider": {
        "skill_file": "skill-aider.md",
        "skill_dst": Path(".aider") / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "copilot": {
        "skill_file": "skill-copilot.md",
        "skill_dst": Path(".copilot") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "claw": {
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".openclaw") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "droid": {
        "skill_file": "skill-droid.md",
        "skill_dst": Path(".factory") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "trae": {
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "trae-cn": {
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae-cn") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "hermes": {
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".hermes") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "kiro": {
        "skill_file": "skill-kiro.md",
        "skill_dst": Path(".kiro") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "pi": {
        "skill_file": "skill-pi.md",
        "skill_dst": Path(".pi") / "agent" / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
    "antigravity": {
        "skill_file": "skill.md",
    ...
    "kimi": {
        "skill_file": "skill.md",
        "skill_dst": Path(".kimi") / "skills" / "graph" / "SKILL.md",
        "claude_md": False,
    },
}
...
def _replace_or_append_section(content: str, marker: str, new_section: str) -> str:
    """Idempotently update or append a graph-owned section in shared files.
...
def install(platform: str = "claude") -> None:
    ...
    if not skill_src.exists():
        print(f"error: {cfg['skill_file']} not found in package - reinstall graph", file=sys.stderr)
        sys.exit(1)

    import os as _os
    if platform in ("claude", "windows") and _os.environ.get("CLAUDE_CONFIG_DIR"):
        _claude_base = Path(_os.environ["CLAUDE_CONFIG_DIR"])
        skill_dst = _claude_base / "skills" / "graph" / "SKILL.md"
    else:
        skill_dst = Path.home() / cfg["skill_dst"]
    ...
    try:
        ...
        try:
            ...
            pass
        raise
    (skill_dst.parent / ".graph_version").write_text(__version__, encoding="utf-8")
...
def _print_install_usage() -> None:
    ...
    print("Usage: graph install [--platform P|P]")
...
This project has a knowledge graph at graph-out/ with god nodes, community structure, and cross-file relationships.

Rules:
- For codebase questions, first run `graph query "<question>"` when graph-out/graph.json exists. Use `graph path "<A>" "<B>"` for relationships and `graph explain "<concept>"` for focused concepts. These return a scoped subgraph, usually much smaller than GRAPH_REPORT.md or raw grep output.
- If graph-out/wiki/index.md exists, use it for broad navigation instead of raw source browsing.
- Read graph-out/GRAPH_REPORT.md only for broad architecture review or when query/path/explain do not surface enough context.
- After modifying code, run `graph update .` to keep the graph current (AST-only, no API cost).
"""

_CLAUDE_MD_MARKER = "## graph"
...
Rules:
- For codebase questions, first run `graph query "<question>"` when graph-out/graph.json exists. Use `graph path "<A>" "<B>"` for relationships and `graph explain "<concept>"` for focused concepts. These return a scoped subgraph, usually much smaller than GRAPH_REPORT.md or raw grep output.
- Dirty graph-out/ files are expected after hooks or incremental updates; dirty graph files are not a reason to skip graph. Only skip graph if the task is about stale or incorrect graph output, or the user explicitly says not to use it.
- If graph-out/wiki/index.md exists, use it for broad navigation instead of raw source browsing.
- Read graph-out/GRAPH_REPORT.md only for broad architecture review or when query/path/explain do not surface enough context.
- After modifying code, run `graph update .` to keep the graph current (AST-only, no API cost).
"""

_AGENTS_MD_MARKER = "## graph"
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                "e=pathlib.Path('graph-out/graph.json').exists();"
                "d={'decision':'allow'};"
                "e and d.update({'additionalContext':'graph: knowledge graph at graph-out/. For focused questions, run `graph query \"<question>\"` (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context.'});"
...
def gemini_install(project_dir: Path | None = None) -> None:
    """Copy skill file to ~/.gemini/skills/graph/, write GEMINI.md section, and install BeforeTool hook."""
    # Copy skill file to ~/.gemini/skills/graph/SKILL.md
    # On Windows, Gemini CLI prioritises ~/.agents/skills/ over ~/.gemini/skills/
    skill_src = Path(__file__).parent / "skill.md"
    if platform.system() == "Windows":
        skill_dst = Path.home() / ".agents" / "skills" / "graph" / "SKILL.md"
    else:
        skill_dst = Path.home() / ".gemini" / "skills" / "graph" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    ...
    if target.exists() and new_content == target.read_text(encoding="utf-8"):
        print(f"graph already configured in {target.resolve()} (no change)")
    else:
        target.write_text(new_content, encoding="utf-8")
        print(f"graph section written to {target.resolve()}")
...
def _install_gemini_hook(project_dir: Path) -> None:
    ...
    settings["hooks"]["BeforeTool"] = [h for h in before_tool if "graph" not in str(h)]
...
def _uninstall_gemini_hook(project_dir: Path) -> None:
    ...
    filtered = [h for h in before_tool if "graph" not in str(h)]
...
def gemini_uninstall(project_dir: Path | None = None) -> None:
    """Remove the graph section from GEMINI.md, uninstall hook, and remove skill file."""
    ...
    if _GEMINI_MD_MARKER not in content:
        print("graph section not found in GEMINI.md - nothing to do")
        return
    cleaned = re.sub(r"\n*## graph\n.*?(?=\n## |\Z)", "", content, flags=re.DOTALL).rstrip()
    if cleaned:
        target.write_text(cleaned + "\n", encoding="utf-8")
        print(f"graph section removed from {target.resolve()}")
    else:
        target.unlink()
...
_VSCODE_INSTRUCTIONS_MARKER = "## graph"
_VSCODE_INSTRUCTIONS_SECTION = """\
## graph

For any question about this repo's architecture, structure, components, or how to add/modify/find
code, your first action should be `graph query "<question>"` when `graph-out/graph.json`
exists. Use `graph path "<A>" "<B>"` for relationship questions and `graph explain "<concept>"`
...
If `graph-out/wiki/index.md` exists, use it for broad navigation. Read `graph-out/GRAPH_REPORT.md`
only for broad architecture review or when query/path/explain do not surface enough context. Only read
source files when (a) modifying/debugging specific code, (b) the graph lacks the needed detail, or
(c) the graph is missing or stale.

Type `/graph` in Copilot Chat to build or update the graph.
"""


def vscode_install(project_dir: Path | None = None) -> None:
    """Install graph skill for VS Code Copilot Chat + write .github/copilot-instructions.md."""
    skill_src = Path(__file__).parent / "skill-vscode.md"
    if not skill_src.exists():
        skill_src = Path(__file__).parent / "skill-copilot.md"
    skill_dst = Path.home() / ".copilot" / "skills" / "graph" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy(skill_src, skill_dst)
    (skill_dst.parent / ".graph_version").write_text(__version__, encoding="utf-8")
    print(f"  skill installed  ->  {skill_dst}")
    ...
    if instructions.exists():
        ...
        if new_content == content:
            ...
            print(f"  {instructions}  ->  graph section {'updated' if _VSCODE_INSTRUCTIONS_MARKER in content else 'added'}")
    else:
        instructions.write_text(_VSCODE_INSTRUCTIONS_SECTION, encoding="utf-8")
        print(f"  {instructions}  ->  created")

    print()
    print("VS Code Copilot Chat configured. Type /graph in the chat panel to build the graph.")
    print("Note: for GitHub Copilot CLI (terminal), use: graph copilot install")


def vscode_uninstall(project_dir: Path | None = None) -> None:
    """Remove graph VS Code Copilot Chat skill and .github/copilot-instructions.md section."""
    skill_dst = Path.home() / ".copilot" / "skills" / "graph" / "SKILL.md"
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"  skill removed    ->  {skill_dst}")
    ...
    if cleaned:
        ...
        print(f"  graph section removed from {instructions}")
    else:
        instructions.unlink()
        print(f"  {instructions}  ->  deleted (was empty after removal)")


_ANTIGRAVITY_RULES_PATH = Path(".agents") / "rules" / "graph.md"
_ANTIGRAVITY_WORKFLOW_PATH = Path(".agents") / "workflows" / "graph.md"

_ANTIGRAVITY_RULES = """\
---
trigger: always_on
description: Consult the graph knowledge graph at graph-out/ for codebase and architecture questions.
---

## graph

This project has a graph knowledge graph at graph-out/.

Rules:
- For codebase or architecture questions, when `graph-out/graph.json` exists, first run `graph query "<question>"` (CLI) or `query_graph` (MCP). Use `graph path "<A>" "<B>"` / `shortest_path` for relationships and `graph explain "<concept>"` / `get_node` for focused concepts. These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
- If graph-out/wiki/index.md exists, navigate it instead of reading raw files
- Read graph-out/GRAPH_REPORT.md only for broad architecture review or when query/path/explain do not surface enough context
- After modifying code files in this session, run `graph update .` to keep the graph current (AST-only, no API cost)
"""
...
Follow the graph skill installed at ~/.agents/skills/graph/SKILL.md to run the full pipeline.
...
graph: A knowledge graph of this project lives in `graph-out/`. \
For codebase, architecture, or dependency questions, when `graph-out/graph.json` exists, \
first run `graph query "<question>"` (or `graph path "<A>" "<B>"` / `graph explain "<concept>"`). \
...
_KIRO_STEERING_MARKER = "graph: A knowledge graph of this project"


def _kiro_install(project_dir: Path) -> None:
    """Write graph skill + steering file for Kiro IDE/CLI."""
    project_dir = project_dir or Path(".")

    # Skill file → .kiro/skills/graph/SKILL.md
    skill_src = Path(__file__).parent / "skill-kiro.md"
    skill_dst = project_dir / ".kiro" / "skills" / "graph" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    skill_dst.write_text(skill_src.read_text(encoding="utf-8"), encoding="utf-8")
    print(f"  {skill_dst.relative_to(project_dir)}  ->  /graph skill")

    # Steering file → .kiro/steering/graph.md (always-on)
    steering_dir = project_dir / ".kiro" / "steering"
    steering_dir.mkdir(parents=True, exist_ok=True)
    steering_dst = steering_dir / "graph.md"
    if steering_dst.exists() and steering_dst.read_text(encoding="utf-8") == _KIRO_STEERING:
        print(f"  .kiro/steering/graph.md  ->  already configured (no change)")
    else:
        # File is wholly graph-owned. Overwrite on upgrade so older
        ...
        print(f"  .kiro/steering/graph.md  ->  always-on steering {action}")

    print()
    print("Kiro will now read the knowledge graph before every conversation.")
    print("Use /graph to build or update the graph.")


def _kiro_uninstall(project_dir: Path) -> None:
    """Remove graph skill + steering file for Kiro."""
    project_dir = project_dir or Path(".")
    removed = []
    ...
    steering_dst = project_dir / ".kiro" / "steering" / "graph.md"
...
def _antigravity_install(project_dir: Path) -> None:
    """Install graph for Google Antigravity: skill + .agents/rules + .agents/workflows."""
    # 1. Copy skill file to ~/.agents/skills/graph/SKILL.md
    ...
    if rules_path.exists():
        ...
        if _ANTIGRAVITY_RULES.strip() != existing.strip():
            ...
            print(f"graph rule updated at {rules_path.resolve()}")
        else:
            print(f"graph rule already configured at {rules_path.resolve()} (no change)")
    else:
        rules_path.write_text(_ANTIGRAVITY_RULES, encoding="utf-8")
        print(f"graph rule written to {rules_path.resolve()}")

    # 3. Write .agents/workflows/graph.md
    ...
    if wf_path.exists():
        ...
        if _ANTIGRAVITY_WORKFLOW.strip() != existing.strip():
            ...
            print(f"graph workflow updated at {wf_path.resolve()}")
        else:
            print(f"graph workflow already configured at {wf_path.resolve()} (no change)")
    else:
        wf_path.write_text(_ANTIGRAVITY_WORKFLOW, encoding="utf-8")
        print(f"graph workflow written to {wf_path.resolve()}")

    print()
    print("Antigravity will now check the knowledge graph before answering")
    print("codebase questions. Run /graph first to build the graph.")
    print()
    print("To enable full MCP architecture navigation, add this to ~/.gemini/antigravity/mcp_config.json:")
    print('  "graph": {')
    print('    "command": "uv",')
    print('    "args": ["run", "--with", "graphy", "--with", "mcp", "-m", "graph.serve", "${workspace.path}/graph-out/graph.json"]')
    print('  }')


def _antigravity_uninstall(project_dir: Path) -> None:
    """Remove graph Antigravity rules, workflow, and skill files."""
    # Remove rules file
    rules_path = project_dir / _ANTIGRAVITY_RULES_PATH
    if rules_path.exists():
        rules_path.unlink()
        print(f"graph rule removed from {rules_path.resolve()}")
    else:
        print("No graph Antigravity rule found - nothing to do")

    # Remove workflow file
    wf_path = project_dir / _ANTIGRAVITY_WORKFLOW_PATH
    if wf_path.exists():
        wf_path.unlink()
        print(f"graph workflow removed from {wf_path.resolve()}")

    # Remove skill file
    skill_dst = _PLATFORM_CONFIG["antigravity"]["skill_dst"]
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"graph skill removed from {skill_dst}")
    version_file = skill_dst.parent / ".graph_version"
    if version_file.exists():
        version_file.unlink()
    for d in (skill_dst.parent, skill_dst.parent.parent, skill_dst.parent.parent.parent):
...
_CURSOR_RULE_PATH = Path(".cursor") / "rules" / "graph.mdc"
_CURSOR_RULE = """\
---
description: graph knowledge graph context
alwaysApply: true
---

This project has a graph knowledge graph at graph-out/.

- For codebase or architecture questions, when `graph-out/graph.json` exists, first run `graph query "<question>"` (or `graph path "<A>" "<B>"` / `graph explain "<concept>"`). These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
- If graph-out/wiki/index.md exists, navigate it instead of reading raw files
- Read graph-out/GRAPH_REPORT.md only for broad architecture review or when query/path/explain do not surface enough context
- After modifying code files in this session, run `graph update .` to keep the graph current (AST-only, no API cost)
"""
...
def _cursor_install(project_dir: Path) -> None:
    """Write .cursor/rules/graph.mdc with alwaysApply: true."""
    ...
    if rule_path.exists() and rule_path.read_text(encoding="utf-8") == _CURSOR_RULE:
        print(f"graph rule at {rule_path} already configured (no change)")
        return
    # File is wholly graph-owned. Overwrite on upgrade so older
    # report-first wording does not silently linger (issue #580).
    action = "updated" if rule_path.exists() else "written"
    rule_path.write_text(_CURSOR_RULE, encoding="utf-8")
    print(f"graph rule {action} at {rule_path.resolve()}")
    print()
    print("Cursor will now always include the knowledge graph context.")
    print("Run /graph . first to build the graph if you haven't already.")


def _cursor_uninstall(project_dir: Path) -> None:
    """Remove .cursor/rules/graph.mdc."""
    rule_path = (project_dir or Path(".")) / _CURSOR_RULE_PATH
    if not rule_path.exists():
        print("No graph Cursor rule found - nothing to do")
        return
    rule_path.unlink()
    print(f"graph Cursor rule removed from {rule_path.resolve()}")
...
_OPENCODE_PLUGIN_JS = """\
// graph OpenCode plugin
// Injects a knowledge graph reminder before bash tool calls when the graph exists.
import { existsSync } from "fs";
import { join } from "path";

export const GraphPlugin = async ({ directory }) => {
  let reminded = false;

  return {
    "tool.execute.before": async (input, output) => {
      if (reminded) return;
      if (!existsSync(join(directory, "graph-out", "graph.json"))) return;

      if (input.tool === "bash") {
        output.args.command =
          'echo "[graph] knowledge graph at graph-out/. For focused questions, run \\`graph query \\"<question>\\"\\` (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context." && ' +
          output.args.command;
        reminded = true;
      }
    },
...
_OPENCODE_PLUGIN_PATH = Path(".opencode") / "plugins" / "graph.js"
_OPENCODE_CONFIG_PATH = Path(".opencode") / "opencode.json"


def _install_opencode_plugin(project_dir: Path) -> None:
    """Write graph.js plugin and register it in opencode.json."""
...
def _uninstall_opencode_plugin(project_dir: Path) -> None:
    """Remove graph.js plugin and deregister from opencode.json."""
    plugin_file = project_dir / _OPENCODE_PLUGIN_PATH
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        # Use the graph CLI itself so the hook is shell-agnostic:
...
def _resolve_graph_exe() -> str:
    """Return the absolute path to the graph executable.

    Falls back to bare 'graph' if resolution fails. Using an absolute path
...
def _install_codex_hook(project_dir: Path) -> None:
    """Add graph PreToolUse hook to .codex/hooks.json."""
    ...
    graph_exe = _resolve_graph_exe()
    hook_entry = {
        "hooks": {
            "PreToolUse": [
                {
                    "matcher": "Bash",
                    "hooks": [{"type": "command", "command": f"{graph_exe} hook-check"}],
                }
            ]
        }
    }
    ...
    existing["hooks"]["PreToolUse"] = [h for h in pre_tool if "graph" not in str(h)]
    existing["hooks"]["PreToolUse"].extend(hook_entry["hooks"]["PreToolUse"])
    hooks_path.write_text(json.dumps(existing, indent=2), encoding="utf-8")
    print(f"  .codex/hooks.json  ->  PreToolUse hook registered ({graph_exe} hook-check)")


def _uninstall_codex_hook(project_dir: Path) -> None:
    """Remove graph PreToolUse hook from .codex/hooks.json."""
    hooks_path = project_dir / ".codex" / "hooks.json"
    ...
    filtered = [h for h in pre_tool if "graph" not in str(h)]
...
def _agents_install(project_dir: Path, platform: str) -> None:
    """Write the graph section to the local AGENTS.md (Codex/OpenCode/OpenClaw)."""
...
def _agents_uninstall(project_dir: Path, platform: str = "") -> None:
    """Remove the graph section from the local AGENTS.md."""
    target = (project_dir or Path(".")) / "AGENTS.md"
    ...
    if _AGENTS_MD_MARKER not in content:
        print("graph section not found in AGENTS.md - nothing to do")
        return
...
def claude_install(project_dir: Path | None = None) -> None:
    """Write the graph section to the local CLAUDE.md."""
...
def _install_claude_hook(project_dir: Path) -> None:
    """Add graph PreToolUse hook to .claude/settings.json."""
    ...
    hooks["PreToolUse"] = [h for h in pre_tool if not (h.get("matcher") in ("Glob|Grep", "Bash") and "graph" in str(h))]
...
def _uninstall_claude_hook(project_dir: Path) -> None:
    """Remove graph PreToolUse hook from .claude/settings.json."""
    settings_path = project_dir / ".claude" / "settings.json"
    if not settings_path.exists():
        return
    try:
    ...
    filtered = [h for h in pre_tool if not (h.get("matcher") in ("Glob|Grep", "Bash") and "graph" in str(h))]
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    """Remove graph from every platform detected in the current project."""
    pd = project_dir or Path(".")
    print("Uninstalling graph from all detected platforms...\n")
    ...
    # Git hook
    try:
        from graph.hooks import uninstall as hook_uninstall
    ...
    if purge:
        ...
        if out.exists():
            _shutil.rmtree(out)
            print(f"\n  graph-out/  ->  deleted (--purge)")
        else:
            print("\n  graph-out/  ->  not found (nothing to purge)")

    print("\nDone. Run 'pip uninstall graphy' to remove the package itself.")


def claude_uninstall(project_dir: Path | None = None) -> None:
    """Remove the graph section from the local CLAUDE.md."""
    target = (project_dir or Path(".")) / "CLAUDE.md"
    ...
    if _CLAUDE_MD_MARKER not in content:
        print("graph section not found in CLAUDE.md - nothing to do")
        return

    # Remove the ## graph section: from the marker to the next ## heading or EOF
    cleaned = re.sub(
        r"\n*## graph\n.*?(?=\n## |\Z)",
        "",
        content,
...
def _clone_repo(url: str, branch: str | None = None, out_dir: Path | None = None) -> Path:
    ...
    Clones into ~/.graph/repos/<owner>/<repo> by default so repeated
    ...
    if out_dir:
        ...
        dest = Path.home() / ".graph" / "repos" / owner / repo
...
def main() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] in ("-v", "--version", "version"):
        print(f"graph {__version__}")
        return

    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        print("Usage: graph <command>")
        print()
        print("Commands:")
        ...
        print("  uninstall               remove graph from all detected platforms in one shot")
        print("    --purge                 also delete graph-out/ directory")
        print("  path \"A\" \"B\"            shortest path between two nodes in graph.json")
        print("    --graph <path>          path to graph.json (default graph-out/graph.json)")
        print("  explain \"X\"             plain-language explanation of a node and its neighbors")
        print("    --graph <path>          path to graph.json (default graph-out/graph.json)")
        print("  clone <github-url>      clone a GitHub repo locally and print its path for /graph")
        ...
        print("    --out <path>            output path (default: graph-out/merged-graph.json)")
        print("    --branch <branch>       checkout a specific branch (default: repo default)")
        print("    --out <dir>             clone to a custom directory (default: ~/.graph/repos/<owner>/<repo>)")
        ...
        print("                            (also: GRAPH_FORCE=1 env var; use after refactors that delete code)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        print("  cluster-only <path>     rerun clustering on an existing graph.json and regenerate report")
        print("    --no-viz                skip graph.html generation (useful for >5000 node graphs / CI)")
        print("    --graph <path>          path to graph.json (default <path>/graph-out/graph.json)")
        print("  query \"<question>\"       BFS traversal of graph.json for a question")
        print("    --dfs                   use depth-first instead of breadth-first")
        print("    --context C             explicit edge-context filter (repeatable)")
        print("    --budget N              cap output at N tokens (default 2000)")
        print("    --graph <path>          path to graph.json (default graph-out/graph.json)")
        print("  save-result             save a Q&A result to graph-out/memory/ for graph feedback loop")
        print("    --question Q            the question asked")
        print("    --answer A              the answer to save")
        print("    --type T                query type: query|path_query|explain (default: query)")
        print("    --nodes N1 N2 ...       source node labels cited in the answer")
        print("    --memory-dir DIR        memory directory (default: graph-out/memory)")
        print("  check-update <path>     check needs_update flag and notify if semantic re-extraction is pending (cron-safe)")
        print("  tree                    emit a D3 v7 collapsible-tree HTML for graph.json")
        print("    --graph PATH            path to graph.json (default graph-out/graph.json)")
        print("    --output HTML           output path (default graph-out/GRAPH_TREE.html)")
        ...
        print("    --out DIR               output dir (default: <path>); writes <DIR>/graph-out/")
        ...
        print("  global add <graph.json>  add/update a project graph in the global graph (~/.graph/global-graph.json)")
        ...
        print("  cursor install          write .cursor/rules/graph.mdc (Cursor)")
        print("  cursor uninstall        remove .cursor/rules/graph.mdc")
        print("  claude install          write graph section to CLAUDE.md + PreToolUse hook (Claude Code)")
        print("  claude uninstall        remove graph section from CLAUDE.md + PreToolUse hook")
        print("  codex install           write graph section to AGENTS.md (Codex)")
        print("  codex uninstall         remove graph section from AGENTS.md")
        print("  opencode install        write graph section to AGENTS.md + tool.execute.before plugin (OpenCode)")
        print("  opencode uninstall      remove graph section from AGENTS.md + plugin")
        print("  aider install           write graph section to AGENTS.md (Aider)")
        print("  aider uninstall         remove graph section from AGENTS.md")
        print("  copilot install         copy graph skill to ~/.copilot/skills (GitHub Copilot CLI)")
        print("  copilot uninstall       remove graph skill from ~/.copilot/skills")
        ...
        print("  claw install            write graph section to AGENTS.md (OpenClaw)")
        print("  claw uninstall          remove graph section from AGENTS.md")
        print("  droid install           write graph section to AGENTS.md (Factory Droid)")
        print("  droid uninstall        remove graph section from AGENTS.md")
        print("  trae install            write graph section to AGENTS.md (Trae)")
        print("  trae uninstall         remove graph section from AGENTS.md")
        print("  trae-cn install         write graph section to AGENTS.md (Trae CN)")
        print("  trae-cn uninstall      remove graph section from AGENTS.md")
        ...
        print("  hermes install          write skill to ~/.hermes/skills/graph/ (Hermes)")
        print("  hermes uninstall        remove skill from ~/.hermes/skills/graph/")
        print("  kiro install            write skill to .kiro/skills/graph/ + steering file (Kiro IDE/CLI)")
        print("  kiro uninstall          remove skill + steering file")
        print("  pi install              write skill to ~/.pi/agent/skills/graph/ (Pi coding agent)")
        print("  pi uninstall            remove skill from ~/.pi/agent/skills/graph/")
        print()
        return
    ...
    if cmd not in _FREE_TEXT_CMDS and any(a in {"-h", "--help", "-?"} for a in sys.argv[2:]):
        print(f"Run 'graph --help' for full usage.")
        return
    ...
    if cmd == "install":
        ...
        if subcmd == "install":
            ...
            print("Usage: graph claude [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "gemini":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graph gemini [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "cursor":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graph cursor [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "vscode":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graph vscode [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "copilot":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graph copilot [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kiro":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graph kiro [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "pi":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            skill_dst = Path.home() / ".pi" / "agent" / "skills" / "graph" / "SKILL.md"
            if skill_dst.exists():
                skill_dst.unlink()
                print(f"  skill removed    ->  {skill_dst}")
            version_file = skill_dst.parent / ".graph_version"
            ...
            print("Usage: graph pi [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print(f"Usage: graph {cmd} [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print("Usage: graph antigravity [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "prs":
        from graph.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from graph.hooks import install as hook_install, uninstall as hook_uninstall, status as hook_status
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graph hook [install|uninstall|status]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "query":
        if len(sys.argv) < 3:
            print("Usage: graph query \"<question>\" [--dfs] [--context C] [--budget N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from graph.serve import _query_graph_text
        ...
        # graph save-result --question Q --answer A --type T [--nodes N1 N2 ...]
        import argparse as _ap
        p = _ap.ArgumentParser(prog="graph save-result")
        p.add_argument("--question", required=True)
        p.add_argument("--answer", required=True)
        p.add_argument("--type", dest="query_type", default="query")
        p.add_argument("--nodes", nargs="*", default=[])
        p.add_argument("--memory-dir", default="graph-out/memory")
        opts = p.parse_args(sys.argv[2:])
        from graph.ingest import save_query_result as _sqr
        out = _sqr(
        ...
        if len(sys.argv) < 4:
            print("Usage: graph path \"<source>\" \"<target>\" [--graph path]", file=sys.stderr)
            sys.exit(1)
        from graph.serve import _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        segments = []
        from graph.build import edge_data
        ...
        if len(sys.argv) < 3:
            print("Usage: graph explain \"<node>\" [--graph path]", file=sys.stderr)
            sys.exit(1)
        from graph.serve import _find_node
        from networkx.readwrite import json_graph
        label = sys.argv[2]
        graph_path = _default_graph_path()
        args = sys.argv[3:]
        ...
        if len(sys.argv) < 3:
            print("Usage: graph add <url> [--author Name] [--contributor Name] [--dir ./raw]", file=sys.stderr)
            sys.exit(1)
        from graph.ingest import ingest as _ingest
        url = sys.argv[2]
        ...
        try:
            ...
            print("Run /graph --update in your AI assistant to update the graph.")
        except Exception as exc:
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from graph.watch import watch as _watch
        try:
            _watch(watch_path)
        ...
        if watch_path is None:
            watch_path = Path(".")
        graph_json = graph_override if graph_override is not None else watch_path / "graph-out" / "graph.json"
        if not graph_json.exists():
            print(f"error: no graph found at {graph_json} — run /graph first", file=sys.stderr)
            sys.exit(1)
        from networkx.readwrite import json_graph as _jg
        from graph.build import build_from_json
        ...
        from graph.export import to_json, to_html
        ...
        out = watch_path / "graph-out"
        out.mkdir(parents=True, exist_ok=True)
        labels_path = out / ".graph_labels.json"
        ...
        from graph.export import _git_head as _gh
        _commit = _gh()
        ...
        from graph.export import backup_if_protected as _backup
        _backup(out)
        ...
        force = os.environ.get("GRAPH_FORCE", "").lower() in ("1", "true", "yes")
        no_cluster = False
        args = sys.argv[2:]
        ...
        if watch_arg is not None:
            ...
            saved = Path(_GRAPH_OUT) / ".graph_root"
            if saved.exists():
        ...
        if not watch_path.exists():
            print(f"error: path not found: {watch_path}", file=sys.stderr)
            sys.exit(1)
        from graph.watch import _rebuild_code
        print(f"Re-extracting code files in {watch_path} (no LLM needed)...")
        # Interactive CLI: block on the per-repo lock rather than skip, so the
        # user sees their explicit `graph update` complete instead of
        ...
        if ok:
            print("Code graph updated. For doc/paper/image changes run /graph --update in your AI assistant.")
            if not (
                os.environ.get("GEMINI_API_KEY")
                or os.environ.get("GOOGLE_API_KEY")
                or os.environ.get("MOONSHOT_API_KEY")
                or os.environ.get("DEEPSEEK_API_KEY")
                or os.environ.get("GRAPH_NO_TIPS")
            ):
        ...
        if len(sys.argv) < 3:
            print("Usage: graph check-update <path>", file=sys.stderr)
            sys.exit(1)
        from graph.watch import check_update
        ...
        from graph.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        graph_path = Path(_GRAPH_OUT) / "graph.json"
        ...
        while i_arg < len(args):
            ...
            if a == "--graph" and i_arg + 1 < len(args):
                ...
                print("Usage: graph tree [--graph PATH] [--output HTML]")
                print("  --graph PATH         path to graph.json (default graph-out/graph.json)")
                print("  --output HTML        output path (default graph-out/GRAPH_TREE.html)")
        ...
        # Usage: graph merge-driver %O %A %B  (set in .git/config merge driver)
        if len(sys.argv) < 5:
            print("Usage: graph merge-driver <base> <current> <other>", file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            print(f"[graph merge-driver] error loading graphs: {exc}", file=sys.stderr)
        ...
        if merged.number_of_nodes() > _MERGE_MAX_NODES:
            print(
                f"[graph merge-driver] merged graph has {merged.number_of_nodes()} nodes, "
        ...
        # graph merge-graphs graph1.json graph2.json ... --out merged.json
        args = sys.argv[2:]
        graph_paths: list[Path] = []
        out_path = Path(_GRAPH_OUT) / "merged-graph.json"
        i = 0
        while i < len(args):
        ...
        if len(graph_paths) < 2:
            print("Usage: graph merge-graphs <graph1.json> <graph2.json> [...] [--out merged.json]", file=sys.stderr)
            sys.exit(1)
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from graph.build import prefix_graph_for_global as _prefix
        graphs = []
        for gp in graph_paths:
            ...
            # Normalize edges/links key before loading — graph writes "links"
        ...
        for G, gp in zip(graphs, graph_paths):
            repo_tag = gp.parent.parent.name  # graph-out/../ → repo dir name
        ...
        if len(sys.argv) < 3:
            print("Usage: graph clone <github-url> [--branch <branch>] [--out <dir>]", file=sys.stderr)
            sys.exit(1)
        url = sys.argv[2]
        ...
        if subcmd not in ("html", "callflow-html", "obsidian", "wiki", "svg", "graphml", "neo4j"):
            print("Usage: graph export <format>", file=sys.stderr)
        ...
        labels_path = Path(_GRAPH_OUT) / ".graph_labels.json"
        labels_path_explicit = False
        report_path = Path(_GRAPH_OUT) / "GRAPH_REPORT.md"
        ...
        analysis_path = Path(_GRAPH_OUT) / ".graph_analysis.json"
        node_limit = 5000
        no_viz = False
        obsidian_dir = Path(_GRAPH_OUT) / "obsidian"
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                print("Usage: graph export callflow-html [GRAPH|DIR] [--graph PATH] [--labels PATH]")
                print("  --report PATH          path to GRAPH_REPORT.md")
                print("  --sections PATH        JSON section definitions")
                print("  --output HTML          output path (default graph-out/<project>-callflow.html)")
                ...
                if candidate.name == "graph.json" or candidate.suffix.lower() == ".json":
                    ...
                    graph_path = candidate / _GRAPH_OUT / "graph.json"
                graph_path_explicit = True
                i += 1
            else:
                i += 1
        ...
        if graph_path_explicit:
            ...
            if not labels_path_explicit:
                labels_path = graph_out_dir / ".graph_labels.json"
        ...
        if not graph_path.exists():
            print(f"error: graph not found: {graph_path}. Run /graph <path> first.", file=sys.stderr)
            sys.exit(1)

        if subcmd == "callflow-html":
            from graph.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from graph.build import build_from_json as _bfj
        ...
        if subcmd == "html":
            from graph.export import to_html as _to_html
            if no_viz:
            ...
            from graph.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from graph.wiki import to_wiki as _to_wiki
            from graph.analyze import god_nodes as _god_nodes
            if not communities:
                print(
                    "error: .graph_analysis.json is missing or empty — refusing to export wiki to prevent data loss.\n"
                    "Run `graph extract .` (or `graph cluster-only .`) to regenerate community data first.",
                    file=sys.stderr,
                )
                sys.exit(1)
            if not gods_data:
            ...
            if neo4j_uri:
                ...
                from graph.export import to_cypher as _to_cypher
                _to_cypher(G, str(out_dir / "cypher.txt"))
                print(f"cypher.txt written - import with: cypher-shell < {out_dir}/cypher.txt")

    elif cmd == "benchmark":
        from graph.benchmark import run_benchmark, print_benchmark
        graph_path = sys.argv[2] if len(sys.argv) > 2 else "graph-out/graph.json"
        # Try to load corpus_words from detect output
        corpus_words = None
        detect_path = Path(".graph_detect.json")
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from graph.global_graph import (
        ...
        if subcmd == "add":
            # graph global add <graph.json> [--as <tag>]
            args = sys.argv[3:]
            source = None
            tag = None
            i = 0
            while i < len(args):
                if args[i] == "--as" and i + 1 < len(args):
                    ...
                    i += 1
            if not source:
                print("Usage: graph global add <graph.json> [--as <repo-tag>]", file=sys.stderr)
                sys.exit(1)
            ...
            if not tag:
                print("Usage: graph global remove <repo-tag>", file=sys.stderr); sys.exit(1)
            try:
            ...
            repos = _global_list()
            if not repos:
                print("Global graph is empty. Use 'graph global add' to add a project.")
            else:
            ...
            print("Usage: graph global [add|remove|list|path]", file=sys.stderr); sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: graph extract <path> [--backend gemini|kimi|claude|openai|deepseek|ollama] "
        ...
        # CLI flag wins over env var. Setting GRAPH_API_TIMEOUT here so
        # _call_openai_compat picks it up without needing a new kwarg path.
        if cli_api_timeout is not None:
            os.environ["GRAPH_API_TIMEOUT"] = str(cli_api_timeout)
        if cli_max_workers is not None:
            os.environ["GRAPH_MAX_WORKERS"] = str(cli_max_workers)
        ...
        # Resolve output dir. The user-facing contract is "<out>/graph-out/"
        # so a fresh checkout writes graph-out/ at the project root, matching
        # the skill.md pipeline.
        out_root = (out_dir.resolve() if out_dir else target)
        graph_out = out_root / "graph-out"
        graph_out.mkdir(parents=True, exist_ok=True)

        from graph.detect import (
            detect as _detect,
            detect_incremental as _detect_incremental,
            save_manifest as _save_manifest,
        )
        manifest_path = graph_out / "manifest.json"
        existing_graph_path = graph_out / "graph.json"
        incremental_mode = manifest_path.exists() and existing_graph_path.exists()

        if incremental_mode:
            print(f"[graph extract] incremental scan of {target}")
            ...
            print(f"[graph extract] scanning {target}")
        ...
        if code_files:
            from graph.extract import extract as _ast_extract
            ast_kwargs: dict = {"cache_root": target}
            if cli_max_workers is not None:
                ast_kwargs["max_workers"] = cli_max_workers
            print(f"[graph extract] AST extraction on {len(code_files)} code files...")
            try:
                ast_result = _ast_extract(code_files, **ast_kwargs)
            except Exception as exc:
                print(f"[graph extract] AST extraction failed: {exc}", file=sys.stderr)
                ast_result = {"nodes": [], "edges": [], "input_tokens": 0, "output_tokens": 0}

        # Semantic extraction on docs/papers/images. Check cache first.
        from graph.cache import (
        ...
        if semantic_files:
            ...
            if sem_cache_hits:
                print(f"[graph extract] semantic cache: {sem_cache_hits} hit / {sem_cache_misses} miss")

            if uncached_paths:
                print(f"[graph extract] semantic extraction on {len(uncached_paths)} files via {backend}...")
                corpus_kwargs: dict = {
                    "backend": backend,
                    "model": model,
                    "root": target,
                ...
                def _progress(idx: int, total: int, _result: dict) -> None:
                    ...
                    print(
                        f"[graph extract] chunk {idx + 1}/{total} done",
                        flush=True,
                    )
                ...
                try:
                    ...
                    print(
                        f"[graph extract] semantic extraction failed: {exc}",
                        file=sys.stderr,
                    )
                ...
                try:
                    ...
                    print(f"[graph extract] warning: could not write semantic cache: {exc}", file=sys.stderr)
        ...
        analysis_path = graph_out / ".graph_analysis.json"
        ...
        if no_cluster:
            ...
            try:
                ...
                print(f"[graph extract] warning: could not write manifest: {exc}", file=sys.stderr)
            if global_merge:
                from graph.global_graph import global_add as _global_add
                _tag = global_repo_tag or target.name
                try:
                    result = _global_add(graph_out / "graph.json", _tag)
                    if result["skipped"]:
                        print(f"[graph global] '{_tag}' unchanged since last add — skipped.")
                    else:
                        print(f"[graph global] '{_tag}' merged into global graph "
                              f"(+{result['nodes_added']} nodes, -{result['nodes_removed']} pruned).")
                except Exception as exc:
                    print(f"[graph global] warning: failed to merge into global graph: {exc}", file=sys.stderr)
            sys.exit(0)
        ...
        from graph.cluster import cluster as _cluster, score_all as _score_all
        from graph.export import to_json as _to_json
        from graph.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        if G.number_of_nodes() == 0:
            print(
                "[graph extract] graph is empty — extraction produced no nodes. "
        ...
        if merged.get("output_tokens", 0) > 0:
            (graph_out / ".graph_semantic_marker").write_text(
        ...
        print(f"[graph extract] wrote {analysis_path}")
        if incremental_mode:
            print(
                f"[graph extract] incremental summary: "
                f"{sem_cache_hits + unchanged_total} files cached/unchanged, "
                f"{len(code_files) + sem_cache_misses} re-extracted, "
                f"{len(deleted_files)} deleted"
            )
        elif sem_cache_hits:
            print(f"[graph extract] semantic cache: {sem_cache_hits} cached, {sem_cache_misses} re-extracted")
        if merged["input_tokens"] or merged["output_tokens"]:
            print(
                f"[graph extract] tokens: "
                f"{merged['input_tokens']:,} in / "
        ...
        # graph cache-check <files_from> [--root <dir>]
        # Reads file paths (one per line) from <files_from>, checks semantic cache.
        # Writes:
        #   graph-out/.graph_cached.json   — already-cached nodes/edges/hyperedges
        #   graph-out/.graph_uncached.txt  — paths that need extraction
        # Stdout: "Cache: N hit, M miss"
        from graph.cache import check_semantic_cache
        if len(sys.argv) < 3:
            print("Usage: graph cache-check <files_from> [--root <dir>]", file=sys.stderr)
            sys.exit(1)
        ...
        if cached_nodes or cached_edges or cached_hyperedges:
            (out / ".graph_cached.json").write_text(
                json.dumps({"nodes": cached_nodes, "edges": cached_edges, "hyperedges": cached_hyperedges},
                           ensure_ascii=False),
                encoding="utf-8",
            )
        (out / ".graph_uncached.txt").write_text("\n".join(uncached), encoding="utf-8")
        print(f"Cache: {len(files) - len(uncached)} hit, {len(uncached)} miss")

    elif cmd == "merge-chunks":
        # graph merge-chunks <chunk_glob_or_files...> --out <path>
        # Concatenates .graph_chunk_*.json files written by semantic subagents.
        # Deduplicates nodes by id (first writer wins). Sums token counts.
        import glob as _glob
        if len(sys.argv) < 3:
            print("Usage: graph merge-chunks <chunk_files...> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        for cf in chunk_files:
            try:
                ...
                print(f"[graph merge-chunks] warning: skipping {cf}: {exc}", file=sys.stderr)
                continue
        ...
        # graph merge-semantic --cached <path> --new <path> --out <path>
        ...
        if len(sys.argv) < 3:
            print("Usage: graph merge-semantic --cached <path> --new <path> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        # User ran `graph <path>` directly — treat as `graph extract <path>`.
        # Common when following the PowerShell note in README (`graph .`) or
        ...
        print("Run 'graph --help' for usage.", file=sys.stderr)
        sys.exit(1)
