...
from graph.google_workspace import (
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the graph extract pipeline.
...
_SKIP_DIRS = {
    ...
    ".graph",  # graph's own extraction cache — never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .graphignore file per gitignore spec.
...
def _load_graphignore(root: Path) -> list[tuple[Path, str]]:
    """Read .graphignore files and return (anchor_dir, pattern) pairs.
    ...
    for d in dirs:
        # Prefer .graphignore; fall back to .gitignore so projects that already
        # maintain a .gitignore get sensible defaults without duplicating it (#945).
        ignore_file = d / ".graphignore"
...
def _is_ignored(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if the path should be ignored per .graphignore patterns.
...
def _load_graphinclude(root: Path) -> list[tuple[Path, str]]:
    """Read .graphinclude allowlist patterns from root and ancestors.
    ...
    Uses the same VCS-root ceiling logic as _load_graphignore.
    """
    root = root.resolve()
    ceiling = _find_vcs_root(root) or root
...
def _is_included(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .graphinclude allowlist pattern."""
    if not patterns:
        return False
...
def _could_contain_included_path(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if a directory may contain files matched by .graphinclude."""
    if not patterns:
        return False
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None) -> dict:
    ...
    # so they win over any .graphignore/.gitignore rules (#947).
    if extra_excludes:
        for pat in extra_excludes:
            line = _parse_gitignore_line(pat)
            if line:
                ignore_patterns.append((root, line))
    include_patterns = _load_graphinclude(root)
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install graphy[office]]")
                continue
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `graph update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `graph extract` after semantic extraction.
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `graph extract` so that
        files touched by `graph update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `graph update`.
