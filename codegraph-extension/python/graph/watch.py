...
def _apply_resource_limits() -> None:
    ...
    GRAPH_REBUILD_MEMORY_LIMIT_MB caps RSS-ish memory. Uses RLIMIT_DATA on
    ...
    try:
        ...
        pass
    mb = os.environ.get("GRAPH_REBUILD_MEMORY_LIMIT_MB", "").strip()
    if not mb:
        return
    try:
...
def _node_community_map(graph_data: dict) -> dict[str, int]:
    ...
    for node in graph_data.get("nodes", []):
        ...
        try:
            ...
            print(
                f"[graph watch] Skipping node with invalid community id: "
...
def _check_shrink(force: bool, existing_data: dict, new_data: dict, tmp: "Path | None" = None) -> bool:
    ...
    if new_n < existing_n:
        ...
        print(
            f"[graph] WARNING: new graph has {new_n} nodes but existing "
...
def _rebuild_code(
    ...
    *,
    ...
    JSON to graph-out/graph.json (mirrors ``extract --no-cluster``).

    Returns True on success, False on error or skipped-due-to-lock.
    """
    out = watch_path / _GRAPH_OUT
    if acquire_lock:
        with _rebuild_lock(out, blocking=block_on_lock) as got:
            if not got:
                print("[graph watch] Rebuild already in progress for "
    ...
    try:
        from graph.extract import extract, _get_extractor
        from graph.detect import detect
        from graph.build import build_from_json
        from graph.cluster import cluster, remap_communities_to_previous, score_all
        ...
        if not code_files:
            print("[graph watch] No code files found - nothing to rebuild.")
            return False
        ...
        if changed_paths is not None:
            ...
            if not wanted and not deleted_paths:
                print("[graph watch] No tracked code files in change set - skipping rebuild.")
                return True
        ...
        (out / ".graph_root").write_text(str(watch_root), encoding="utf-8")
        ...
        if no_cluster:
            ...
            try:
                from graph.detect import save_manifest
            ...
            if same_graph:
                print("[graph watch] No code-graph changes detected (--no-cluster); outputs left untouched.")
            else:
                print(
                    "[graph watch] Rebuilt (no clustering): "
                    f"{len(result.get('nodes', []))} nodes, {len(result.get('edges', []))} edges"
                )
                print(f"[graph watch] graph.json updated in {out}")
            return True
        ...
        if existing_graph_data:
            ...
            if same_topology:
                ...
                if flag.exists():
                    flag.unlink()
                print("[graph watch] No code-graph topology changes detected; outputs left untouched.")
                return True
        ...
        labels_file = out / ".graph_labels.json"
        try:
        ...
        if no_change:
            ...
            print("[graph watch] No code-graph changes detected; graph.json/GRAPH_REPORT.md left untouched.")
        else:
        ...
        if not no_change:
            try:
                ...
                print(f"[graph watch] Skipped graph.html: {viz_err}")
        ...
        if callflow_files and not no_change:
            try:
                from graph.callflow_html import write_callflow_html
                for cf in callflow_files:
                    write_callflow_html(
                        graph=out / "graph.json",
                        report=out / "GRAPH_REPORT.md",
                        labels=out / ".graph_labels.json",
                        output=cf,
                        verbose=False,
                    )
            except Exception as cf_err:
                print(f"[graph watch] callflow HTML update skipped: {cf_err}")
        ...
        if not no_change:
            print(f"[graph watch] Rebuilt: {G.number_of_nodes()} nodes, "
            ...
            print(f"[graph watch] {products} updated in {out}")
        return True

    except Exception as exc:
        print(f"[graph watch] Rebuild failed: {exc}")
        return False
...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/graph --update` — this function only signals
    that the update is needed.
    """
    flag = Path(watch_path) / _GRAPH_OUT / "needs_update"
    if flag.exists():
        print(f"[graph check-update] Pending non-code changes in {watch_path}.")
        print("[graph check-update] Run `/graph --update` to apply semantic re-extraction.")
    return True


def _notify_only(watch_path: Path) -> None:
    """Write a flag file and print a notification (fallback for non-code-only corpora)."""
    flag = watch_path / _GRAPH_OUT / "needs_update"
    flag.parent.mkdir(parents=True, exist_ok=True)
    flag.write_text("1", encoding="utf-8")
    print(f"\n[graph watch] New or changed files detected in {watch_path}")
    print("[graph watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[graph watch] Run `/graph --update` in Claude Code to update the graph.")
    print(f"[graph watch] Flag written to {flag}")
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /graph --update (LLM extraction required).
    ...
    # Load .graphignore patterns ONCE at startup so the handler does not
    ...
    ignore_patterns = _load_graphignore(watch_root_for_ignore)
    ...
    class Handler(FileSystemEventHandler):
        def on_any_event(self, event):
            ...
            # Check .graphignore BEFORE the extension/dotfile/out filters so
    ...
    print(f"[graph watch] Watching {watch_path.resolve()} - press Ctrl+C to stop")
    print(f"[graph watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /graph --update.")
    print(f"[graph watch] Debounce: {debounce}s")
    ...
    try:
        while True:
            ...
            if pending and (time.monotonic() - last_trigger) >= debounce:
                ...
                batch = list(changed)
                changed.clear()
                print(f"\n[graph watch] {len(batch)} file(s) changed")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the graph graph")
