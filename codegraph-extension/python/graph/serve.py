...
def _subgraph_to_text(G: nx.Graph, nodes: set[str], edges: list[tuple], token_budget: int = 2000, *, seeds: list[str] | None = None) -> str:
    ...
    for nid in ordered:
        ...
        # corpus document can otherwise inject ANSI escapes, fake graph-out
...
def serve(graph_path: str = "graph-out/graph.json") -> None:
    ...
    def _tool_list_prs(arguments: dict) -> str:
        from graph.prs import fetch_prs, fetch_worktrees, format_prs_text, _detect_default_branch
    ...
    def _tool_get_pr_impact(arguments: dict) -> str:
        from graph.prs import fetch_pr_files, compute_pr_impact, _gh, _parse_ci
    ...
    def _tool_triage_prs(arguments: dict) -> str:
        ...
        from graph.prs import fetch_prs, fetch_worktrees, fetch_pr_files, compute_pr_impact, _STATUS_ORDER, _detect_default_branch
        repo = arguments.get("repo") or None
        base = arguments.get("base") or _detect_default_branch(repo)
        try:
            prs = fetch_prs(repo=repo, base=base)
    ...
    def _load_community_labels() -> dict[int, str]:
        labels_path = Path(graph_path).parent / ".graph_labels.json"
    ...
    async def list_resources() -> list[types.Resource]:
        return [
            types.Resource(uri=AnyUrl("graph://report"), name="Graph Report", description="Full GRAPH_REPORT.md", mimeType="text/markdown"),
            types.Resource(uri=AnyUrl("graph://stats"), name="Graph Stats", description="Node/edge/community counts and confidence breakdown", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("graph://god-nodes"), name="God Nodes", description="Top 10 most-connected nodes", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("graph://surprises"), name="Surprising Connections", description="Cross-community surprising connections", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("graph://audit"), name="Confidence Audit", description="EXTRACTED/INFERRED/AMBIGUOUS edge breakdown", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("graph://questions"), name="Suggested Questions", description="Suggested questions for this codebase", mimeType="text/plain"),
        ]
    ...
    async def read_resource(uri: AnyUrl) -> str:
        ...
        if uri_str == "graph://report":
            ...
            return "GRAPH_REPORT.md not found. Run graph extract first."
        if uri_str == "graph://stats":
            return _tool_graph_stats({})
        if uri_str == "graph://god-nodes":
            return _tool_god_nodes({"top_n": 10})
        if uri_str == "graph://surprises":
            try:
                from graph.analyze import surprising_connections
        ...
        if uri_str == "graph://questions":
            try:
                from graph.analyze import suggest_questions
