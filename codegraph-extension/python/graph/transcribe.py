...
_DEFAULT_MODEL = "base"
_TRANSCRIPTS_DIR = "graph-out/transcripts"
_FALLBACK_PROMPT = "Use proper punctuation and paragraph breaks."


def _model_name() -> str:
    return os.environ.get("GRAPH_WHISPER_MODEL", _DEFAULT_MODEL)
...
def download_audio(url: str, output_dir: Path) -> Path:
    ...
    """
    from graph.security import validate_url
...
def build_whisper_prompt(god_nodes: list[dict]) -> str:
    ...
    domain hint from these labels and passes it via GRAPH_WHISPER_PROMPT or
    as initial_prompt — no separate API call needed here.
    """
    if not god_nodes:
        return _FALLBACK_PROMPT

    override = os.environ.get("GRAPH_WHISPER_PROMPT")
    if override:
        return override
